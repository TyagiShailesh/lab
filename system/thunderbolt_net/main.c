 ...
 *
 * Performance patches: page_pool RX, larger ring, threaded NAPI
 */
...
/* Higher NAPI weight for better throughput */
#define TBNET_NAPI_WEIGHT	256
 ...
 * @pool: Page pool for fast RX buffer recycling
 ...
 * @login_received: ThunderboltIP login message received from the remote host
 ...
 * @disconnect_work: Worker that handles tearing down the ThunderboltIP connection
 * @rx_hdr: Copy of the currently processed Rx frame
...
/* Free RX buffers using page_pool — pages are recycled, not freed */
static void tbnet_free_rx_buffers(struct tbnet *net)
{
	...
	for (i = 0; i < TBNET_RING_SIZE; i++) {
		...
		page_pool_put_full_page(net->pool, tf->page, false);
		tf->page = NULL;
	}
...
/*
 * Allocate RX buffers using page_pool. The page_pool pre-maps pages for
 * DMA so we avoid the per-packet dma_map_page/dma_unmap_page overhead.
 */
static int tbnet_alloc_rx_buffers(struct tbnet *net, unsigned int nbuffers)
{
	...
	while (nbuffers--) {
		...
		page = page_pool_dev_alloc_pages(net->pool);
		if (!page)
			return -ENOMEM;
...
static int tbnet_poll(struct napi_struct *napi, int budget)
{
	...
	while (rx_packets < budget) {
		...
		/* Return some buffers to hardware in batches */
		...
		/* No dma_unmap_page needed — page_pool handles DMA lifecycle */
		...
		if (!tbnet_check_frame(net, tf, hdr)) {
			...
			page_pool_recycle_direct(net->pool, page);
...
static int tbnet_open(struct net_device *dev)
{
	...
	/* Create page pool for fast RX buffer recycling */
	net->pool = page_pool_create(&pp_params);
	if (IS_ERR(net->pool)) {
		int err = PTR_ERR(net->pool);
		net->pool = NULL;
		netdev_err(dev, "failed to create page pool: %d\n", err);
...
static int tbnet_probe(struct tb_service *svc, const struct tb_service_id *id)
{
	...
	/* Use threaded NAPI with higher weight for better batching */
	netif_napi_add_weight(dev, &net->napi, tbnet_poll, TBNET_NAPI_WEIGHT);
...
MODULE_DESCRIPTION("Thunderbolt/USB4 network driver (page_pool fast path)");
